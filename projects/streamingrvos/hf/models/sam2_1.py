...
class SAM2(nn.Module):
    ...
    def build_image_encoder_tiny(self):
        def build_trunk():
            embed_dim = 96
            num_heads = 1
            stages = [1, 2, 7, 2]
    ...
    def inject_language_embd(self, inference_state, language_embd):
        ...
        for frame_idx in range(num_frame):
            ...
            for obj_idx in range(num_obj):
                ...
                _, _, out_mask_logits = self.sam2_model.add_language_embd_ious(inference_state, frame_idx, obj_idx + 100, _language_embd)
    ...
    def language_embd_inference(self, inference_state, language_embd):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for frame_idx in range(num_frame):
                ...
                for obj_idx in range(num_obj):
                    _language_embd = language_embd[frame_idx][obj_idx][None][None]
                    _, _, out_mask_logits, ious, object_score_logits = self.sam2_model.add_language_embd_ious(
                        inference_state,
                        frame_idx,
                        obj_idx + 100,
                        _language_embd,
            ...
            # mask_out = torch.cat(mask_out, dim=0)
        return mask_out, ious, object_score_logits
...
class SAM2Base(SAM2Base_):
    ...
    def track_step(
        ...
        current_out = {"point_inputs": point_inputs, "mask_inputs": mask_inputs}
        # High-resolution feature maps for the SAM head, reshape (HW)BC => BCHW
        if len(current_vision_feats) > 1:# go
            high_res_features = [
                x.permute(1, 2, 0).view(x.size(1), x.size(2), *s)
                for x, s in zip(current_vision_feats[:-1], feat_sizes[:-1])
            ]
            ...
            high_res_features = None
        if mask_inputs is not None and self.use_mask_input_as_output_without_sam:#pass
            # When use_mask_input_as_output_without_sam=True, we directly output the mask input
            # (see it as a GT mask) without using a SAM prompt encoder + mask decoder.
            pix_feat = current_vision_feats[-1].permute(1, 2, 0)
            pix_feat = pix_feat.view(-1, self.hidden_dim, *feat_sizes[-1])
            ...
            pix_feat_with_mem = self._prepare_memory_conditioned_features(
                ...
                num_frames=num_frames,
                track_in_reverse=track_in_reverse,
            )# pix_feat_with_mem.shape:[1,256,64,64]
            # apply SAM-style segmentation head
            # here we might feed previously predicted low-res SAM mask logits into the SAM mask decoder,
            # e.g. in demo where such logits come from earlier interaction instead of correction sampling
            # (in this case, any `mask_inputs` shouldn't reach here as they are sent to _use_mask_as_output instead)
            if prev_sam_mask_logits is not None:# pass
                assert point_inputs is not None and mask_inputs is None
                mask_inputs = prev_sam_mask_logits
            multimask_output = self._use_multimask(is_init_cond_frame, point_inputs)# True, None, multimask_output:True
            sam_outputs = self._forward_sam_heads(
                backbone_features=pix_feat_with_mem,
                point_inputs=point_inputs,
                mask_inputs=mask_inputs,
        ...
        # Finally run the memory encoder on the predicted mask to encode
        # it into a new memory feature (that can be used in future frames)
        if run_mem_encoder and self.num_maskmem > 0:# pass
            high_res_masks_for_mem_enc = high_res_masks
            maskmem_features, maskmem_pos_enc = self._encode_new_memory(
                current_vision_feats=current_vision_feats,
                feat_sizes=feat_sizes,
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:# go
            # Cache miss -- we will run inference on a single image
            # image = inference_state["images"][frame_idx].cuda().float().unsqueeze(0)
            image = inference_state["images"][frame_idx].cuda().unsqueeze(0)# [1,3,h,w]
            backbone_out = self.forward_image(image)# backbone_out:dict 
    ...
    def _run_single_frame_inference(
        ...
        maskmem_pos_enc = _get_maskmem_pos_enc(inference_state, current_out)# maskmem_pos_enc: None
        # object pointer is a small tensor, so we always keep it on GPU memory for fast access
        obj_ptr = current_out["obj_ptr"]# obj_ptr [1, 256]
        ...
        return compact_current_out, pred_masks_gpu, ious, object_score_logits
    ...
    def add_language_embd_ious(
        ...
        current_out, pred_mask_gpu, ious, object_score_logits = self._run_single_frame_inference(
            inference_state=inference_state,
        ...
        if inference:
            ...
            # )
        return frame_idx, obj_ids, pred_mask_gpu, ious, object_score_logits
