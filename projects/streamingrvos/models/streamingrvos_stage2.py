...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def forward(self, data, data_samples=None, mode='loss', fast_token_idx=None, time_context_prompt=None):
        ...
        outputs = self._llm_forward(
            ...
            time_context_prompt=time_context_prompt,
            is_video=is_video,
    ...
    def _llm_forward(
        ...
        # add time context
        if time_context_prompt is not None:
            input_embeds[time_context_token_id] = time_context_prompt
        ...
        seg_token_embed_video = outputs.hidden_states[-1][seg_mask.reshape(B, -1)].clone().detach() #[4, 896]
         

        # if time_context_prompt is None:

        logits = outputs.logits.clone().detach()# video: [4,302,151682] img: [4,3507,151682]

        loss = None
        if labels is not None: # img label [4, 3507]
            ...
            # self.model.language_model.config.vocab_size: 151682
        ...
        # else:
            ...
            # loss = torch.tensor(1.).to(outputs.logits.device)
        
        # 对img和video分开处理
        if is_video:
            return loss, seg_token_embed_video
        else:
            output = CausalLMOutputWithPast(
                loss=loss,
                logits=logits,
                past_key_values=outputs.past_key_values,#video:None, img:None
                hidden_states=outputs.hidden_states,#video tuple len=25 [4,302,896], img: tuple len=25 [4,3507,896]
                attentions=outputs.attentions,#video:None, img:None
            )
