...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def __init__(self,
        ...
        time_context_token_id = tokenizer.convert_tokens_to_ids('<TIME_CONTEXT>')
        self.model.time_context_token_id = time_context_token_id
    ...
    def forward(self, data, data_samples=None, mode='loss', fast_token_idx=None):
        if 'fast_pixel_values' in data.keys():# pass
        ...
        # data['input_ids'].shape [B, 1992]
        pixel_values = data['pixel_values'] # video:[B, 5, 3, 448, 448] img: len=1d的list [,3,448,448]
        ...
        if type(pixel_values) is list or pixel_values.ndim == 5:
            ...
            # video:b*n, c, h, w, img:[img1+img2, c, h, w]
        ...
        position_ids = data['position_ids']# [B,]
    ...
    def _llm_forward(
        ...
        # input_ids vqa: [2, 367]  videoseg:[2, 446] error: vqa时input_embeds 是nan
        vocab_size = self.model.language_model.get_input_embeddings().weight.shape[0]
        if (input_ids >= vocab_size).any() or (input_ids < 0).any():
            raise ValueError("Input IDs contain out-of-vocabulary indices")
        input_embeds = self.model.language_model.get_input_embeddings()(input_ids).clone()

        if is_video:
            self.time_context_prompt = None
            pixel_values = pixel_values.reshape(batch_size, -1,  *pixel_values.shape[1:]).permute(1,0,2,3,4)  # (n, b, c, h, w)
            image_flags = image_flags.reshape(batch_size, -1).permute(1, 0)  # (n, b) 

            for idx, pixel_value in enumerate(pixel_values): 
                if fast_pixel_values is not None:# pass
                    print("oh! oh! oh! fast_pixel_values is not None in video !!!")
                else:
                    vit_embeds = self.model.extract_feature(pixel_value)# pixel_values.shape:[bs*5, 3, 448, 448], vit_embeds.shape:[bs*5, 256, 896]

                    vit_embeds = vit_embeds.to(input_embeds.dtype)  # FIXME: why vit_embeds is float16?
                    fast_vit_embeds = None


                vit_embeds = vit_embeds[image_flags[idx] == 1] # [b, 256, 896]
                vit_batch_size = pixel_value.shape[0]

                B, N, C = input_embeds.shape# sample 5 frame[b, 1458, 896] 
                # input_embeds = input_embeds.reshape(B * N, C) #[,896]

                self._count += 1

                if vp_overall_mask is not None and prompt_masks is not None:# pass
                    print("oh! vp_overall_mask is not Done in line 320!!")
                else:
                    vp_embeds = None

                # input_ids = input_ids.reshape(B * N)
                selected = (input_ids == self.model.img_context_token_id)
                time_context_token_id = (input_ids == self.model.time_context_token_id)
                seg_mask = (input_ids == self.seg_token_idx)
                # for video qa
                # time_context_end_id = time_context_token_id[0].nonzero().item() + 1            
                ...
                if vp_embeds is None:
                    ...
                    print("oh! vp_embeds is not None in line 346 !!!")
                    try:
                ...
                if fast_vit_embeds is not None:#pass
                ...
                # add time context
                if self.time_context_prompt is not None:
                    input_embeds[time_context_token_id] = self.time_context_prompt
                ...
                # 这里应该只要最后一个
                # seg_token_embed_list.append(outputs.hidden_states[-1][seg_mask].unsqueeze(1))

                # 分别处理，这里一个batch可能含有seg和qa两种数据
                time_context_prompt_list = []
                for i in range(batch_size):
                    if seg_mask[i].sum() !=0: # 有<seg>的数据，只用传递<seg>
                        # 7-21 modify
                        # time_context_prompt = outputs.hidden_states[-1][i][seg_mask[i]].reshape(-1, C)# (5, 896) 5个seg标记的不一定是同一个！！
                        # time_context_prompt = torch.mean(time_context_prompt, dim=0).clone().detach() #[896]

                        time_context_prompt = outputs.hidden_states[-1][i][seg_mask[i]].reshape(-1, C).squeeze(0).clone().detach()
                        if time_context_prompt.dim() != 1: # img video 混杂
                            time_context_prompt = time_context_prompt[0]
                        time_context_prompt_list.append(time_context_prompt)

                self.time_context_prompt = torch.stack(time_context_prompt_list, dim=0)
                # outputs.logits.shape [2,447, 151682], hidden_states len=25, [0].shape:[2, 447, 896], attentions:None, past_key_values:None
            
            seg_token_embed_video = outputs.hidden_states[-1][seg_mask] # [B*5, 896]
            # seg_token_embed_video = torch.cat(seg_token_embed_list, dim=1) # (B, 5, 896)

        else:# img 保持不变
            if fast_pixel_values is not None:# pass
                # print("oh! oh! oh! fast_pixel_values is not None !!!")
                ...
                vit_embeds = self.model.extract_feature(pixel_values)# pixel_values.shape:[bs*5, 3, 448, 448], vit_embeds.shape:[bs*5, 256, 896]

                vit_embeds = vit_embeds.to(input_embeds.dtype)  # FIXME: why vit_embeds is float16?
                fast_vit_embeds = None

            vit_embeds = vit_embeds[image_flags == 1] # [13+7, 256, 896]
            vit_batch_size = pixel_values.shape[0]# [13+7]

            B, N, C = input_embeds.shape
            input_embeds = input_embeds.reshape(B * N, C) #[7020, 896]

            self._count += 1

            if vp_overall_mask is not None and prompt_masks is not None:# pass
                # print("oh! vp_overall_mask is not Done!!")
                vp_embeds = []
                vp_overall_mask = vp_overall_mask.to(vit_embeds.device).bool()
                prompt_masks = [item.to(vit_embeds.device).bool() for item in prompt_masks] # len =2  [1,16,16] [4,16,16]

                vp_overall_mask = vp_overall_mask[image_flags == 1] # shape:[2] 不变
                overall_tile_vit_embeds = vit_embeds[vp_overall_mask] # (n_img, hw, c) 不变

                i_vp_img = 0
                for i_img in range(len(vit_embeds)):
                    vp_embeds.append(vit_embeds[i_img].reshape(-1, C))
                    if vp_overall_mask[i_img]:
                        tile_vit_embeds = overall_tile_vit_embeds[i_vp_img].reshape(-1, C)  # [256, 896]
                        objects_prompt_masks = prompt_masks[i_vp_img] # [ , 16, 16]
                        n_obj = len(objects_prompt_masks)
                        tile_vit_embeds = tile_vit_embeds.unsqueeze(0).repeat(n_obj, 1, 1) # [n_obj, 256, 896]
            ...
            input_ids = input_ids.reshape(B * N)# B:2 N:3510
            selected = (input_ids == self.model.img_context_token_id) #7020 其中5120个1
            ...
            if vp_embeds is None:
                ...
                try:
                    ...
                    n_token = selected.sum()
                    if n_token > len(vp_embeds):
                        print(f"Wrong !!! {n_token} image tokens in text but only {len(vp_embeds)} vp embeds !!!")
                        expand_ratio = n_token // len(vp_embeds) + 1
                        vp_embeds = torch.cat([vp_embeds] * expand_ratio, dim=0)
            ...
            outputs = self.model.language_model(
                ...
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
            )# img: logits:[2, 1985, 151682], hidden_states len=25, [0].shape:[2, 1985, 896], attentions:None, past_key_values:None
            seg_token_embed_video = None
        ...
        loss = None
        if labels is not None: # img label [2, 1992],video: [B, 447]
        ...
        return output, seg_token_embed_video
