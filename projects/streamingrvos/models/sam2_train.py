...
class SAM2TrainRunner(BaseModule):
    def __init__(
            self,
            cfg_path: str = "sam2_1_hiera_l.yaml",
            ckpt_path: str = "sam2.1_hiera_large.pt",
            # cfg_path: str = "sam2.1_hiera_large.yaml",
            # ckpt_path: str = "sam2.1_hiera_large.pt",
        ...
        import third_parts.sam2_1 # noqa: F401

        if hydra_overrides_extra is None:
            hydra_overrides_extra = []
        hydra_overrides = [
            ## Extension: LLM prompt
            "++model._target_=projects.streamingrvos.models.extension.SAM2Base",
        ]
    ...
    def inject_language_embd_sam2_1(self, sam_states, language_embd):
        high_res_features = [
            x.permute(1, 2, 0).view(x.size(1), x.size(2), *s)
            for x, s in zip(sam_states['current_vision_feats'][:-1], sam_states['feat_sizes'][:-1])
        ]
        ...
        if self.sam2_model.directly_add_no_mem_embed:
            ...
            raise NotImplementedError("directly add no memory embedding is not implemented")
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            _, _, ious, low_res_masks, high_res_masks, obj_ptr, object_score_logits, = self.sam2_model._forward_sam_heads(
                                            backbone_features=pix_feat_with_mem,
                                            point_inputs=None,
                                            mask_inputs=None,
                                            high_res_features=high_res_features,
            ...
            pred_masks = low_res_masks
        return pred_masks, ious, object_score_logits
