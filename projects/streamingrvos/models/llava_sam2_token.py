...
class VideoLLaVASAMModel_zero3(VideoLLaVASAMModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        # data[in_put_ids].shape = [B,3534] 前6个：27(<), 91(|), 872(user), 91(|), 397(>), 151665(<img>), 后面就是若干个151667(<IMG_CONTEXT>), 151666(</img>), 198......
        g_pixel_values = data.pop('g_pixel_values', None)# 对于图片是len(g_pixel_values) == bs，视频是数据集取的个数*bs,qa数据集是None, 即没有额外的图片输入，QA时图片输入放在inputs_id里
        ...
        input_ids = data['input_ids']# sav:[B,1770]

        is_video = data['is_video']

        if is_video:
            if self.fast_pool:# pass
                print("self.fast_pool is not Done!!!")
                output = self.mllm(data, data_samples, mode, fast_token_idx=self.fast_token_idx)
            else:
                output, seg_token_embed_video = self.mllm(data, data_samples, mode)# seg_token_embed_video: [B, 5, 896]
        else:# img
            if self.fast_pool:# pass
                output = self.mllm(data, data_samples, mode, fast_token_idx=self.fast_token_idx)
            else:
        ...
        if is_video:
            ...
            seg_token_embed_video = self.text_hidden_fcs(seg_token_embed_video)# seg_token_embed_video [B*5, 256]
            
            # print(f"hidden_states: {hidden_states.shape}, seg_token_mask: {seg_token_mask.shape}, seg_valid: {seg_valid}")
            _zero = hidden_states.mean() * 0.0
            if seg_valid:
                pred_embeddings = seg_token_embed_video + _zero# [B*5, 256]
            else:
                pred_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero# keep, not changed
            # pred_embeddings.shape[B*2, 256]
        ...
        pred_masks = self.grounding_encoder.inject_language_embd(sam_states, language_embeddings, nf_nobj=(num_frames, num_objs))
        # pred_masks [B*num_frames,num_expression,256,256]
