...
from projects.streamingrvos.models.preprocess.image_resize import DirectResize
...
class VideoLLaVASAMModel_zero3(VideoLLaVASAMModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        # data: 'input_ids' 'attention_mask' 'position_ids' 'labels' 'frames_per_batch' 'g_pixel_values' 'masks'
        g_pixel_values = data.pop('g_pixel_values', None)# len=B*frames [3,1024,1024]
        gt_masks = data.pop('masks', None)# video: len(gt_masks)=1, gt_masks[0].shape = [25, imgw, imgh]，QA数据集是None
        frames_per_batch = data.pop('frames_per_batch', None)# image:[1] video:[5]，QA数据集是[]
        assert frames_per_batch, "Video Lisa require frames_per_batch !!!"
        input_ids = data['input_ids']
        ...
        if gt_masks is None:
            ...
            g_pixel_values, frames_per_batch, gt_masks = self._get_pesudo_data(
                dtype=self.torch_dtype,
                device=input_ids.device,# 对于QA数据集，g_pixel_values，gt_masks全是0，frames_per_batch [1]
            )
        else:
            seg_valid = True
        ...
        if is_video:
            ...
            g_pixel_values = torch.stack([self.grounding_encoder.preprocess_image(pixel) for pixel in g_pixel_values])# [20,3,1024,1024]
            B = input_ids.size(0)
            len_frame = data['pixel_values'].size(1)
            g_pixel_values = g_pixel_values.reshape(B, len_frame, g_pixel_values.size(1), g_pixel_values.size(2),g_pixel_values.size(3)).permute(1,0,2,3,4)# [5,4,3,h,w]
            gt_masks_video = self.process_video_gt_masks(gt_masks, frames_per_batch)

            self.pred_iou = 0.
            self.seg_token_embed_video = None
            self.prompt_token = None
            self.total_loss = 0.
            mask_list = []
            llm_count = 0
            
            split_pixel_values = torch.unbind(data['pixel_values'], dim=1)# list len=5 [4,3,448,448]
            for i in range(len_frame):
                per_g_pixel_values = g_pixel_values[i]
                # 拆开 data的pixel_values, 只改变这一个值，其余不变
                data['pixel_values'] = [t.unsqueeze(0) for t in torch.unbind(split_pixel_values[i], dim=0)]
                if self.pred_iou < 0.7:
                    llm_count += 1
                    # 
                    loss, seg_token_embed_video = self.mllm(data, data_samples, mode, time_context_prompt=self.prompt_token)# seg_token_embed_video: [B, 1,896]
                    self.prompt_token = seg_token_embed_video.clone()
                    
                    seg_token_embed_video = self.text_hidden_fcs(seg_token_embed_video)# seg_token_embed_video [B, 256]
                    seg_token_embed_video = seg_token_embed_video.unsqueeze(1)#[4,1,256]
                    self.seg_token_embed_video = seg_token_embed_video.clone()

                    sam_states = self.grounding_encoder.get_sam2_embeddings(per_g_pixel_values)
                    pred_masks, ious, _ = self.grounding_encoder.inject_language_embd_sam2_1(sam_states, self.seg_token_embed_video)# pred_masks [B,1,256,256]

                    self.pred_iou = ious.max().item()
                    ...
                    # 直接sam2 分割出结果
                    sam_states = self.grounding_encoder.get_sam2_embeddings(per_g_pixel_values)
                    # sam_states['current_vision_feats']: len 为3，[65536, 5, 32] [16384, 5, 64] [4096, 5, 256]. language_embeddings.shape: [bs*frames*5, 1, 256]
                    pred_masks, ious, _ = self.grounding_encoder.inject_language_embd_sam2_1(sam_states, self.seg_token_embed_video.clone())# pred_masks [B*num_frames,num_expression,256,256]
                    self.pred_iou = ious.max().item()
                    mask_list.append(pred_masks)

            self.llm_loss = self.total_loss / llm_count 
            pred_masks = torch.stack(mask_list, dim=1)
            pred_masks = pred_masks.squeeze(2).clone()

        else:# img  data['pixel_values'] len=B,[0].shape:[1,3,448,448]
            if self.fast_pool:# pass
                output = self.mllm(data, data_samples, mode, fast_token_idx=self.fast_token_idx)
            else:
                output, _ = self.mllm(data, data_samples, mode)
            ...
            seg_token_mask = input_ids == self.seg_token_idx#img：一个True，对于video，seg_token_mask有5个True，都在后面，对于QA 全是False

            hidden_states = output.hidden_states# tuple,img:len(hidden_states)=25,
            hidden_states = self.text_hidden_fcs(hidden_states[-1])# hidden_states [B, ,256]
            
            # print(f"hidden_states: {hidden_states.shape}, seg_token_mask: {seg_token_mask.shape}, seg_valid: {seg_valid}")
            _zero = hidden_states.mean() * 0.0
            if seg_valid:
                pred_embeddings = hidden_states[seg_token_mask] + _zero
            else:
                pred_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero
            # img, video:pred_embeddings.shape[B*2, 256]
            seg_token_counts = seg_token_mask.int().sum(-1)# img, video [5]
            if not seg_valid:
                seg_token_counts += 5

            pred_embeddings_list_ = torch.split(pred_embeddings, seg_token_counts.tolist(), dim=0)# tuple len=B, [0].shape:[5,256]
            pred_embeddings_list = []
            for item in pred_embeddings_list_:
                if len(item) != 0:
                    pred_embeddings_list.append(item)
            # len(pred_embeddings_list)=2, [0].shape [5, 256]
            pred_embeddings_list_video, success = self.genetate_video_pred_embeddings(
                pred_embeddings_list, frames_per_batch)
            # len(pred_embeddings_list_video)=B*5, [0].shape:[5,256]
            if not success:
                raise NotImplementedError
                # return {'llm_loss': output.loss, 'loss_mask': output.loss * 0.0, 'loss_dice': output.loss * 0.0}
            # len(gt_masks)=B, [0].shape [num_frames*num_expresion, 480, 640]
            gt_masks_video = self.process_video_gt_masks(gt_masks, frames_per_batch)# video:len(gt_masks)=B, frames_per_batch=5, len(gt_masks_video)=5, img:len(gt_masks)=B, frames_per_batch [1, 1] len(gt_masks_video)=B
            # len(gt_masks_video)=B*num_frames, [0].shape [5, h, w]
            pred_embeddings_list_video, gt_masks_video = self.check_obj_number(
                pred_embeddings_list_video, gt_masks_video
            ) # video: len(pred_embeddings_list_video)=B*5, [5,256] len(gt_masks_video)=B*5 [5,1280,720]
            g_pixel_values = torch.stack([
                self.grounding_encoder.preprocess_image(pixel) for pixel in g_pixel_values
            ])# video:对5张图片进行resize之后再压成一个batch，对于QA全是0,img:len(g_pixel_values)=1
            # 
            num_objs = pred_embeddings_list_video[0].shape[0]# 5
            num_frames = len(pred_embeddings_list_video) # B*5
            language_embeddings = torch.cat(pred_embeddings_list_video, dim=0)[:, None]#[bs*frames*5,1,256]
            
            sam_states = self.grounding_encoder.get_sam2_embeddings(g_pixel_values, expand_size=num_objs)
            # g_pixel_values [4,3,1024,1024], language_embeddings [20,1,256]
            pred_masks = self.grounding_encoder.inject_language_embd(sam_states, language_embeddings, nf_nobj=(num_frames, num_objs))
            # pred_masks [4,5,256,256]
