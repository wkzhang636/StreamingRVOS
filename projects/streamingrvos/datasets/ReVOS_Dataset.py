...
class VideoReVOSDataset(Dataset):
    ...
    TIME_CONTEXT_TOKEN = '<TIME_CONTEXT>'
    TIME_START_TOKEN = '<context>'
    TIME_END_TOKEN = '</context>'
    ...
    def prepare_text(self, n_frames, expressions, num_image_tokens=256, n_fast_images=50, n_time_context=1):
        ...
        for i, (question, answer) in enumerate(zip(questions, answers)):
            if i == 0:
                ...
                context_tokens = f'{self.TIME_START_TOKEN}' \
                                    f'{self.TIME_CONTEXT_TOKEN * n_time_context}'\
                                    f'{self.TIME_END_TOKEN}'

                qa_list.append(
                    {'from': 'human', 'value': frame_tokens + context_tokens + question + after_question_str}
                )
            else:
                qa_list.append(
    ...
    def __getitem__(self, index):
        ...
        if data_dict.get('images', None) is not None:
            ...
            if self.preprocessor is not None:#pass
                if self.arch_type == 'qwen':
